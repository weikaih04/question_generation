...
SINGLE_OBJ_VERBS = ['v024', 'v011', 'v013', 'v029',
                    'v027', 'v015', 'v007', 'v002']
...
def addObjects(stsg, obj, arel, crel, srel, vrel, refs, metrics):
    ...
    #     print("    Has exactly one: ", [ENG[r] for r in exactly_one])
    # if len(more_than_one) > 0:
    #     print("    Has more than one: ", [ENG[r] for r in more_than_one])

   # print("RELS", [ENG[r] for r in rels])

    #print('    before adding objects: ', sums(refs))
    refs = addFirstObjRef(stsg, obj, arel, more_than_one, [], [], refs)
    #print('    after adding first: ', sums(refs))
    refs = addLastObjRef(stsg, obj, arel,  more_than_one, [], [], refs)
    #print('    after adding last: ', sums(refs))
    # refs = addTimeObjRef(stsg, obj, arel,  more_than_one, [], [], refs)
    refs = addSingleObjRef(stsg, obj, arel,  exactly_one, [], [], refs)
    #print('    after adding single: ', sums(refs))

    if True: # TODO print put back to normal
        for o in obj:
            for tp in refs[o]:
                if len(refs[o][tp]) > 3:
...
def addRelations(stsg, obj, arel, crel, srel, vrel, refs, metrics):
    ...
    for r in rels:
        refs[r] = {
            'direct': [makeRef(phrase=g.PP[r],
                               program=lambda a: a[0],
                               steps=0,
                               args=[r],
                               tp='direct',
                               direct_type=False,
        ...
        for tp in refs[action_id]:
            for indirect in refs[action_id][tp]:
                ...
                if (first_frame != video_first_frame):
                    ...
                    phrases = [
                        " before starting to %s" % (PRES[indirect['direct']]),
                    ...
                    sg_phrases = [[PRES[indirect['direct']]],
                                  [indirect['phrase']]]
                ...
                for a_id2 in act:
                    for tp in refs[a_id2]:
                        for indirect2 in refs[a_id2][tp]:
                            ...
                            phrases = [
                                " before %s but after %s" %
                                (indirect2['phrase'], indirect1['phrase']),
                                " between %s and %s" %
    ...
    for tp in ['all']:
        for indirect in refs['time'][tp]:
            for r in rels:
                ...
                phrases = [
                    "the object they were %s first%s" % (g.PP[r],
                                                         indirect['phrase']),
                    "the first thing they %s%s" % (PAST[r],
                                                   indirect['phrase']),
                ]
    ...
    for tp in ['all']:
        for indirect in refs['time'][tp]:
            for r in rels:
                ...
                phrases = [
                    "the object they were %s last%s" % (g.PP[r],
                                                        indirect['phrase']),
...
def addSingleObjRef(stsg, obj, arel, crel, srel, vrel, refs):
    ...
    #print("in single, rels are: ", [ENG[r] for r in rels])
    #interest = False
    for r in rels:
        # if ENG[r] == 'leaning on':
        #     #print("leaning on!")
...
def addFirstRelRef(stsg, obj, arel, crel, srel, vrel, refs):
    ...
    for o in obj:
        ...
        tree = 'firstRel'
        refs = add(refs, direct, 'first', phrase,
                   program_first, 1, args, metrics, tree)
        ...
        for j in dic[i]:
            ...
            for k in dic[i][j]:
                ...
                new_inst = {
                    ...
                    'direct': k['direct'],
                    'type': k['type'],
                    'indirects': k['metrics']['indirects']
                }

                new_r[i][j].append(new_inst)
        print()
    return new_r
...
def printAll():
    ...
    for video_id in ids:
        print(video_id)
        r = makeIndirect(STSG[video_id])
        r2 = printDic(r)

        refs_dic[video_id] = r2

    print('saving!!!', type(refs_dic))
    with open('../data/testing_compo_ref-new.txt', 'w+') as f:
        json.dump(refs_dic, f)
