...
def objExists(stsg, a):
    ...
    def frames_func(f_id, args):
        # If change, make sure to change in solve action indirect
        for i in ['contact', 'verb']:
            contact = pm.select(stsg, ['frames', f_id, i, 'vertices'])
    ...
    # CHANGED
    sg_objects = (stsg['stsg']['objects'].keys())
    for o_vert in list(set(o_verts)):
        print(o_vert)
        if o_vert not in sg_objects:
            print(o_vert)
